...
class CampaignService {
  ...
  public async verifyAndQueueCampaigns(): Promise<void> {
    ...
    try {
      ...
      if (campaigns.length > 0) {
        logger.info(
          `[Campañas] Verificación encontró ${campaigns.length} campañas programadas.`
        );
        for (const campaign of campaigns) {
          ...
          const delay = moment(campaign.scheduledAt).diff(
            moment(),
            "milliseconds"
          );
          campaignQueue.add(
            "ProcessCampaign",
            { id: campaign.id },
            { delay: Math.max(0, delay), priority: 3 }
          );
          logger.info(
            `[Campañas] Campaña ${campaign.id} encolada para procesar.`
          );
        }
      }
    } catch (err) {
    ...
    try {
      ...
      if (isArray(contacts) && contacts.length > 0) {
        ...
        for (let i = 0; i < contacts.length; i++) {
          ...
          const interval =
            i > settings.longerIntervalAfter
        ...
        logger.info(
          `[Campañas] ${contacts.length} contactos de la campaña ${campaignId} han sido encolados para preparación.`
        );
      }
    } catch (err: any) {
      Sentry.captureException(err);
      logger.error(
        `[Campañas] Error procesando campaña ${campaignId}: ${err.message}`
      );
    }
  }

  public async prepareContactForCampaign(
    ...
    try {
      ...
      if (!campaign || !contact) {
        logger.warn(
          `[Campañas] No se encontró campaña o contacto. Campaña: ${campaignId}, Contacto: ${contactId}`
        );
        return;
      }
      ...
      Sentry.captureException(err);
      logger.error(
        `[Campañas] Error preparando contacto ${contactId} para campaña ${campaignId}: ${err.message}`
      );
    }
  }

  public async dispatchCampaignMessage(
    data: DispatchCampaignData
  ): Promise<void> {
    ...
    try {
      ...
      if (!campaign || !campaign.whatsapp || !campaignShipping) {
        logger.error(
          `[Campañas] Datos insuficientes para despacho. Campaña: ${campaignId}, Envío: ${campaignShippingId}`
        );
        return;
      }
      ...
      if (campaign.openTicket === "enabled") {
        await this._dispatchWithMessageAndTicket(
          wbot,
          campaign,
          campaignShipping,
          chatId
        ...
        await this._dispatchSimpleMessage(
          wbot,
          campaign,
          campaignShipping,
          chatId
      ...
      logger.error(
        `[Campañas] Error al despachar mensaje de campaña ${campaignId}: ${err.message}`
      );
  ...
  /**
   * Verifica si un mensaje entrante es una respuesta de confirmación para una campaña activa.
   * Si lo es, procesa la confirmación.
   * @param message - El mensaje entrante de Baileys.
  ...
  public async checkConfirmation(
    ...
    if (campaigns) {
      ...
      if (campaignShipping) {
        ...
        // Vuelve a encolar el siguiente paso de la campaña
        campaignQueue.add("DispatchCampaign", {
  ...
  /**
   * Verifica si un mensaje saliente es parte de una campaña para cerrar el ticket asociado.
   * @param message - El mensaje saliente de Baileys.
   * @param companyId - El ID de la compañía.
   * @param wbot - La instancia de la conexión de WhatsApp.
   */
  public async checkMessageAndCloseTicket(
    message: WAMessage,
    companyId: number,
    wbot: WASocket
  ): Promise<void> {
    const body = getBodyMessage(message); // Asumiendo que getBodyMessage está en un helper
  ...
  private async _dispatchSimpleMessage(
    wbot: Session,
    campaign: Campaign,
    ...
    logger.info(
      `[Campañas] Mensaje simple de campaña ${campaign.id} enviado a ${campaignShipping.contact.name}`
    );
  }

  private async _dispatchWithMessageAndTicket(
    wbot: Session,
    campaign: Campaign,
    campaignShipping: CampaignShipping,
    chatId: string
    ...
    if (!campaign.mediaPath) {
      ...
      await CreateTextMessage(sentMessage, fullTicket, contact, null);
    } else {
      const filePath = path.resolve(
        "public",
        `company${campaign.companyId}`,
    ...
    await campaignShipping.update({ deliveredAt: moment().toDate() });
    logger.info(
      `[Campañas] Mensaje con ticket de campaña ${campaign.id} enviado a ${campaignShipping.contact.name}`
    );
  }

  private _getInitialMessage(
    campaign: Campaign,
    variables: any[],
    ...
    const message = this._getProcessedMessage(
