...
const downloadMedia = async (msg: proto.IWebMessageInfo, isImported: Date = null, wbot: Session, ticket: Ticket) => {
  ...
  if (wbot && message?.fileLength && +message.fileLength > fileLimit * 1024 * 1024) {
    ...
    sendMsg.message.extendedTextMessage.text = "\u200e*Mensaje del sistema*:\nArchivo recibido excede el límite de tamaño del sistema. Si es necesario, puede obtenerlo en la aplicación de WhatsApp.";
...
const verifyQueue = async (
  ...
  const botText = async () => {
    ...
    if (choosenQueue || (queues.length === 1 && chatbot)) {
      ...
      await wbot.presenceSubscribe(contact.remoteJid);
  ...
  const botList = async () => {
    ...
    if (choosenQueue || (queues.length === 1 && chatbot)) {
      ...
      if (choosenQueue.chatbots.length > 0 && !ticket.isGroup) {
        ...
        await CreateTextMessage(sendMsg, ticket, contact, ticketTraking);
          ...
          async () => {
            try {
              ...
              buttons.push({
                name: 'quick_reply',
                buttonParamsJson: JSON.stringify({
                  display_text: "Volver al Menú Inicial",
                  id: "#"
                })
              });
  ...
  try {
        ...
        if (!isNil(whatsapp.collectiveVacationMessage && !isGroup)) {
          ...
          if (
            ...
            if (hasMedia) {
              ...
              await CreateTextMessage(msg, ticket, contact, ticketTraking);
            }
    ...
    if (!useLGPD) {
      ...
      if (hasMedia) {
        ...
        mediaSent = await CreateMediaMessage(
          msg,
          ticket,
          contact,
          ticketTraking,
        ...
        // console.log("antes do CreateTextMessage")
    ...
    if (
      ...
      const sentMessage = await wbot.sendMessage(
        `${contact.number}@c.us`,
        {
          text: `\u200e*Asistente Virtual*:\nLamentablemente, no podemos escuchar ni enviar audios por este canal de atención. Por favor, envíe un mensaje de *texto*.`
        },
      {
          quoted: {
            key: msg.key,
